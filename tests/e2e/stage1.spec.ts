...
async function createMarkdownFixture(useHome = false): Promise<string> {
  const baseDir = useHome ? homedir() : tmpdir();
  const dir = join(baseDir, `md-viewer-e2e-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
test('stage 1 shell starts securely and reads Markdown through the preload API', async () => {
  const fixturePath = await createMarkdownFixture(false);
  try {
    ...
    await rm(dirname(fixturePath), { recursive: true, force: true }).catch(() => {});
  }
});

test('opens a Markdown file passed as a launch argument without test authorization', async () => {
  const fixturePath = await createMarkdownFixture(true);
  try {
    const appPath = join(process.cwd(), 'out/main/index.js');
    const electronApp = await electron.launch({
